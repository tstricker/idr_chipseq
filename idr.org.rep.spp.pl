...
foreach my $foo (keys %fin){
    ...
    my $out = $data_dir."/consistency/".$foo;
    `mkdir $out`;
    
    print OUT "setpkgs -a R\n";
    print OUT "cd /home/strickt2/SEQanalysis/phantompeakqualtools/idrCode\n";
    print OUT "Rscript $batch_spp $fin{$foo}[0] $fin{$foo}[1] -1 $out 0 F signal.value \n";
    print "Rscript $batch_spp $fin{$foo}[0] $fin{$foo}[1] -1 $out 0 F signal.value \n";
