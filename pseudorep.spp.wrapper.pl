...
foreach my $foo (keys %chip){
   ...
   # unless ($foo =~ /TPS-280/){
    ...
    my $out = $data_dir."/".$foo."selfPseudo/".$foo;
    my $out1 = $out."00";
    my $out2 = $out."01";
    my $out_dir = $data_dir."/".$foo."selfPseudo/";
    #`mkdir $out_dir`;
    my $pool = $out."/".$foo.".pooled.tagAlign.gz";
    print OUT "setpkgs -a R\n";
    #print " \$nlines = (zcat -f $fin{$foo} | wc -l)\n";
    my $nlines = `zcat -f $fin{$foo} | wc -l`;
    print "zcat -f $fin{$foo} | wc -l\n";
    my $nlines1 = ($nlines+1)*"0.5";
    
    my $nlines2 =  sprintf("%.0f", $nlines1);
    print "$nlines2\t$nlines1\t$nlines\n";
    #my $nlines = 5000;
    print OUT "zcat $foo $fin{$foo} | shuf | split -d -l $nlines2 - $out\n";
    print OUT "gzip $out1\n";
    print OUT "gzip $out2\n";
    print OUT "mv $out1.gz $out.pr1.tagAlign.gz\n";
    print OUT "mv $out2.gz $out.pr2.tagAlign.gz\n";
    print OUT "Rscript $run_spp -c=$out.pr1.tagAlign.gz -i=$chip{$foo}{INPUT} -npeak=300000 -odir=$out_dir -savr -savp -rf\n";  
    print OUT "Rscript $run_spp -c=$out.pr2.tagAlign.gz -i=$chip{$foo}{INPUT} -npeak=300000 -odir=$out_dir -savr -savp -rf\n";
